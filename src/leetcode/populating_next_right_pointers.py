...
class Solution:
    ...
    def connect(self, root):
        ...
        self.__node_dict = {}
        self.__traverse(root, 0, 1)
        return root

    def __traverse(self, node, row, col):
        if (row, col) in self.__node_dict:
            self.__node_dict[(row, col)].next = node

        self.__node_dict[(row, col + 1)] = node

        if node.right is not None:
            self.__traverse(node.left, row + 1, col * 2)
            self.__traverse(node.right, row + 1, col * 2 + 1)
